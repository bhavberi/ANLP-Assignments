...
perplexity_save_path = "2021111013-LM2-{}-perplexity.txt"
...
model_save_path = "language_model_q2.pth"
...
class LSTMLanguageModel(nn.Module):
    def __init__(
        ...
        # self.hidden = self.init_hidden(batch_size, num_layers, hidden_dim)
    ...
    def init_hidden(self, batch_size, num_layers, hidden_dim):
        ...
        cell = torch.zeros(num_layers, batch_size, hidden_dim).to(device)
        return hidden, cell
...
# %%
def get_perplexity(model, data, vocab, filename, criterion, sentences, embedding_matrix):
    ...
    with torch.no_grad():
        for sentence in tqdm(data, disable=tqdm_disable):
            ...
            sentence_vectors = torch.zeros((max_length-1, embedding_dim), dtype=torch.float, device=device)
...
model = LSTMLanguageModel(
    vocab_size, embedding_dim, hidden_dim, embedding_matrix, num_layers=1, dropout=0.3
).to(device)
...
get_perplexity(
    model, val_data, vocab, perplexity_save_path.format("val"), criterion, val_sentences, embedding_matrix
)
get_perplexity(
    model,
    test_data,
